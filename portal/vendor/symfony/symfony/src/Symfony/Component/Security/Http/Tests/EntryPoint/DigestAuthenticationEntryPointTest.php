...
class DigestAuthenticationEntryPointTest extends TestCase
{
    public function testStart()
    {
        ...
        $this->assertRegExp('/^Digest realm="TheRealmName", qop="Security", nonce="[a-zA-Z0-9\/+]+={0,2}"$/', $response->headers->get('WWW-Authenticate'));
    }
    ...
    public function testStartWithNonceExpiredException()
    {
        ...
        $this->assertEquals(401, $response->getStatusCode());
        $this->assertRegExp('/^Digest realm="TheRealmName", qop="Security", nonce="[a-zA-Z0-9\/+]+={0,2}", stale="true"$/', $response->headers->get('WWW-Authenticate'));
    }
}
